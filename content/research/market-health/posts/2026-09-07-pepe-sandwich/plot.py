...
def main():
    ...
    ax.set(xlim=(0, 18), ylim=(0, 102), xlabel="PEPE output shortfall versus the fixed-input replay (%)",
    ...
    ax.set_title("115 purchases in 58 exact-inventory sequences | PEPE/WETH | 18–19 April 2023 UTC", fontsize=10, loc="left", pad=13, fontweight="normal")
    ...
    ax.set_title("58 exact-inventory sequences | 18–19 April 2023 UTC | not a net-profit estimate", fontsize=10, loc="left", pad=13, fontweight="normal")
    ...
    first = episodes[0]
    v = next(v for v in victims if v["episode_id"] == first["episode_id"])
    ...
    ax.set(ylabel="PEPE received (billions)", ylim=(0, 110))
    ax.grid(axis="y", color=GRID); ax.set_axisbelow(True)
    ax.set_title("Same input: 3.610091 WETH", fontsize=10, loc="left", fontweight="normal")
    fig.suptitle("A purchase bracketed by an exact-inventory round trip", x=.08, ha="left", fontsize=15, fontweight="bold")
    fig.text(.08, .89, "Ethereum block 17,070,040 · transaction indices 7 → 8 → 9 · 18 April 2023, 00:29:23 UTC", fontsize=10)
